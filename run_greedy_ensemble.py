...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Caruana-style greedy ensemble selection (with replacement)."
    )
    parser.add_argument("--input", required=True, help="Input CSV path")
    parser.add_argument(
        "--output",
        default="ensemble_output.csv",
        help="Output CSV path with ensemble predictions",
    )
    parser.add_argument(
        "--report-path",
        default="ensemble_report.json",
        help="Output JSON path with selected models and history",
    )
    parser.add_argument("--id-col", default="ID", help="ID column name")
    parser.add_argument("--label-col", default="Label", help="Label column name")
    parser.add_argument(
        "--pred-prefix", default="pred_", help="Prefix for prediction columns"
    )
    parser.add_argument(
        "--pred-columns",
        default=None,
        help="Comma-separated prediction columns. If set, pred-prefix is ignored.",
    )
    parser.add_argument(
        "--metric",
        default="rmse",
        choices=["rmse", "mae", "r2", "spearman", "pcc", "pearson", "logloss", "auc"],
        help="Optimization metric",
    )
    parser.add_argument(
        "--maximize",
        action="store_true",
        help="Force maximize metric (default behavior depends on metric)",
    )
    parser.add_argument(
        "--n-iterations",
        type=int,
        default=100,
        help="Maximum number of greedy additions",
    )
    parser.add_argument(
        "--early-stopping-rounds",
        type=int,
        default=None,
        help="Stop if no improvement for this many rounds",
    )
    parser.add_argument(
        "--tol",
        type=float,
    ...
    parser.add_argument(
        "--no-use-best-iteration",
        action="store_true",
        help="Use full iteration history instead of cutting at best iteration",
    )
...
def main() -> None:
    ...
    ensembler = CaruanaGreedyEnsembler(
        ...
        tol=args.tol,
        use_best_iteration=not args.no_use_best_iteration,
